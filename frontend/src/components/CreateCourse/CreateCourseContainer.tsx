...
const CreateCourseContainer = (): React.ReactElement => {
  ...
  const [typeCreate, setTypeCreate] = React.useState<object | undefined>();
  // data
...
const LessonInformation = (props: {
  ...
  typeCreate: object | undefined;
  setTypeCreate: React.Dispatch<React.SetStateAction<object | undefined>>;

  dataSection: ISection[];
  setDataSection: React.Dispatch<React.SetStateAction<ISection[]>>;
}): React.ReactElement => {
  const handlerCloseModal = () => {
    props.setIsOpenModal(false);
  };

  const handlerAddNewLesson = () => {
    props.setDataSection((prev) => [...prev, { id: 1, title: "Section 1" }]);
  };
  return (
    <>
      <SectionList setIsOpenModal={props.setIsOpenModal} />
      <div
        id="popup-modal"
        tabIndex={-1}
                ...
                <div className="flex flex-col  gap-2">
                  ...
                  <div className="flex items-center justify-start gap-4">
                    <LessonTypeItem
                      ...
                      onClick={() => {
                        props.setTypeCreate({ type: "lesson" });
                      }}
                    />
                    <LessonTypeItem
                      icon={<MonitorPlay />}
                      title="bài học video"
                      onClick={() => {
                        props.setTypeCreate({ type: "video" });
                      }}
                    />
                  </div>
                </div>
